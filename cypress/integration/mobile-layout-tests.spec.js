...
context("touch and phone layout", () => {
	...
	context("on a touch device", () => {
		...
		it("grows the palette swatches past the minimum target size", () => {
		...
		it("wraps the palette into more rows so every color still fits on screen", () => {
			cy.window().then((win) => {
				expect(palette_rows(win), "palette rows").to.be.greaterThan(2);
				expect(
					win.document.querySelectorAll(".palette .color-button").length,
					"swatches in the palette"
				).to.equal(win.default_palette.length);
